...
let preguntas = readText("/triviate/preguntas/generalKnowledge.json");
...
function guardarPuntaje(user,puntaje){
    ...
    fs.collection('rankingConocimiento').doc(`${username}`).get().then((snapshot) =>{
        if(snapshot.data()){
            if (scoreData.points > snapshot.data().points){
                fs.collection('rankingConocimiento').doc(`${username}`).set(scoreData);
            }
        }else{
            fs.collection('rankingConocimiento').doc(`${username}`).set(scoreData);
        }
